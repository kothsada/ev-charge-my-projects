...
| Admin | 4000 | `panda_ev_system` | IAM, stations, pricing |
...
npm run test
npm run test:cov
npx jest src/modules/notification/notification-flow.integration.spec.ts
...
| `RABBITMQ_OCPP_EVENTS_NOTI_QUEUE` | `PANDA_EV_QUEUE_NOTI` | Dedicated OCPP fanout copy for aggregation (not `PANDA_EV_QUEUE`) |
| `RABBITMQ_FCM_CLEANUP_QUEUE` | `PANDA_EV_FCM_CLEANUP` | Publish stale token events back to Mobile API |
...
Mobile API ──publish──► PANDA_EV_NOTIFICATIONS ──► NotificationRouter ──► NotificationProcessor
                         (notification.targeted /        │                   (dedup → rate-limit
                          ...
                          notification.overstay_reminder) │                    → WebSocket emit)
                                                          │
OCPP CSMS ──publish──► PANDA_EV_QUEUE_NOTI ─────────────►│ (aggregation + live dashboard only)
           (fanout copy; PANDA_EV_QUEUE goes to Mobile)

Mobile/CSMS ──publish──► PANDA_EV_SMS ──────────────────► SmsRouter ──► SmsService ──► LTC API
              (sms.otp / sms.text)

NotificationProcessor ──► PANDA_EV_FCM_CLEANUP ──► Mobile API (stale FCM tokens)
```
...
**Stale token flow:** When FCM reports a token as invalid, `NotificationProcessor` calls `DeviceService.markTokensStale()` and also publishes `{ routingKey: 'device.token_stale', fcmTokens[] }` to `PANDA_EV_FCM_CLEANUP` so Mobile API can clean up its own device table.
...
**Module load order in `app.module.ts`**: `ServiceAuthModule` must appear before `RabbitMQModule` — `RabbitMQService` injects `ServiceJwtService` at construction time. `ScheduleModule.forRoot()` is also registered globally.
...
3. **Token resolution** — uses `fcmTokens[]` if provided, else queries `user_fcm_devices` by `userId`
4. **FCM send** — multicast to resolved tokens
5. **Stale token cleanup** — marks stale tokens in DB; publishes `device.token_stale` to `PANDA_EV_FCM_CLEANUP`
6. **DB log** — write `NotificationLog` record (soft-fail)
7. **Aggregation** — UPSERT to `notification_daily_stats`
8. **WebSocket** — emit `notification:sent` to `/admin-stats`

### RabbitMQ queues consumed

`NotificationRouter` registers consumers in `onModuleInit`. `SmsRouter` registers the SMS consumer in its own `onModuleInit`. All consumers are stored internally and replayed automatically after reconnect.
...
| `PANDA_EV_QUEUE_NOTI` | Plain consume (no DLQ) | `handleOcppEvent()` — aggregation + WebSocket emit only |
| `PANDA_EV_SMS` | With DLQ; 3 retries at 5s / 30s / 120s | `handleSmsMessage()` — routes by `routingKey` |

**`consume()` and `consumeWithDlq()` are synchronous** (return `void`, not `Promise<void>`). Do not `await` them — consumers are registered immediately and started when a channel is available.
...
### Database schema (`panda_ev_noti`)

Key models:

| Model | Table | Notes |
|---|---|---|
| `NotificationLog` | `notification_logs` | One record per FCM send; unique on `(sessionId, type)` |
| `UserFcmDevice` | `user_fcm_devices` | Canonical token store; `isActive=false` = soft-deleted stale token |
| `UserNotificationPreference` | `user_notification_preferences` | Per-user opt-in flags + quiet hours (not yet wired into pipeline) |
| `NotificationTemplate` | `notification_templates` | i18n templates (lo/en/zh); seeded via `prisma/seed/seed-templates.ts` |
| `NotificationDailyStat` | `notification_daily_stats` | Unique on `(date, type, channel)` — UPSERT only |
| `StationHourlyStat` | `station_hourly_stats` | Unique on `(stationId, hour)` — UPSERT only |
| `StationDailyStat` | `station_daily_stats` | Unique on `(stationId, date)` — UPSERT only |
| `SmsLog` | `sms_logs` | One record per LTC submit; `transactionId` is unique |
| `SmsDailyStat` | `sms_daily_stats` | Unique on `date` — UPSERT only |
...
`ServiceJwtService` (global, `src/common/service-auth/`) — RS256 30-second tokens with Redis jti anti-replay. Redis key format: `svc:jti:{serviceName}:{jti}` (namespaced so fanout deliveries of the same token to multiple services don't false-positive as replays). Generate keys:
