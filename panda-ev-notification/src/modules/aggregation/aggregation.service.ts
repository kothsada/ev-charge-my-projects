...
import { startOfHour, toVientianeDateStr } from '../../common/helpers/date.helper';
...
export class AggregationService {
  ...
  async onSessionCompleted(event: SessionCompletedEvent): Promise<void> {
    try {
      ...
      const dateStr = toVientianeDateStr(event.completedAt);
      ...
      await this.prisma.$executeRaw`
        ...
        VALUES
          (gen_random_uuid(), ${event.stationId}, ${event.stationName}, ${dateStr}::date, 1, 1, ${energyDecimal}::decimal, ${BigInt(Math.round(event.amountLak))}, ${event.durationMinutes.toFixed(2)}::decimal, ${overstayLak}, ${wasOverstay})
  ...
  async onNotificationSent(
    ...
    try {
      const todayStr = toVientianeDateStr(new Date());
      ...
      await this.prisma.$executeRaw`
        ...
        VALUES
          (gen_random_uuid(), ${todayStr}::date, ${type}, ${channelEnum}::"panda_ev_noti"."NotificationChannel", ${sentIncr}, ${failedIncr}, ${suppressedIncr})
  ...
  async onOverstayCharged(event: OverstayChargedEvent): Promise<void> {
    try {
      ...
      await this.prisma.$executeRaw`
        ...
        VALUES
          (gen_random_uuid(), ${event.stationId}, ${event.stationName}, ${dateStr}::date, ${overstayLak}, 1)
        ON CONFLICT (station_id, date) DO UPDATE SET
          total_overstay_lak = station_daily_stats.total_overstay_lak + ${overstayLak},
